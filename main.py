...
def is_find_word_k(word, group):
    total_sum = 0
    for arr in group:
        if word[0] == arr[0] and word[len(word) - 1] == arr[len(arr) - 1]:  # to allow insertions only in the middle
            j = 1
...
def main(cog, q, l, k):
    ...
    # create a Counter with key = word, value = the size of the group that the organism appears in it (q)
    counter1 = Counter()

    # create a hashmap with the word (a tuple) as a key and array of [number_of_appears, set_of_organism] as the value
    cog_map = {}
    ...
    if k == 0:  # solution from part A
        ...
        output = []  # to clear the output vector
        ...
        # print('Time of part2: ', time_stop2-time_stop1)
